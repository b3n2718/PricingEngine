...
py::array_t<double> GBM::evolve(
    ...
    double dt) const
{
    auto s  = state.unchecked<1>();  // current spot prices, shape [n_sims]
    ...
    // Pre-compute the constant parts of the log-return for this step.
    // Using the exact log-normal solution avoids discretisation error:
    //   S(t+dt) = S(t) · exp(drift·dt + σ·√dt · Z)
    double drift     = (params_.risk_free_rate - params_.div_yield
                        - 0.5 * params_.vol * params_.vol) * dt;
    double diffusion = params_.vol * std::sqrt(dt);

    for (int i = 0; i < n; ++i) {
        r(i) = s(i) * std::exp(drift + diffusion * zr(i, 0));
    }
    return result;
}

py::array_t<double> GBM::initial_state(int n_sims) const {
    // All paths start at the same spot price.
